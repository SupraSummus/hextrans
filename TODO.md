...
`test_building_build_house_random` — cleanup asserts
`tool_remove_way` over (7,9)→(9,9) returns `null`, but the city
auto-generated by `tool_add_city` at (8,8) on pak128 doesn't lay
road there, so `two_click_tool_t::work` returns `""` ("nothing to
remove") instead of `null` (`simmenu.cc:1411`).  Test invariant
(build/remove a random house) survives; the cleanup just needs to
build the road explicitly before removing it rather than depending
on city content.
