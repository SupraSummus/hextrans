...
/*
 * Executor-side bodies for the nwc_* command classes.
 *
 * network_cmd_ingame.cc holds the wire-parser side (constructors,
 * rdwr() bodies, read_from_packet dispatch).  This file holds the
 * execute() / clone() / do_command() bodies, plus the karte_t-typed
 * constructor of nwc_tool_t and a few private helpers, all of which
 * reach into karte_t / env_t::nickname / GUI / loadsave / scripts.
 * Splitting these out keeps the parser side NETTOOL-buildable —
 * fuzz_network and (eventually) nettool link only the .cc on the
 * parser side and supply vtable anchors in fuzz_stubs.cc.
 */
