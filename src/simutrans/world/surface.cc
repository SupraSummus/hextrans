...
slope_t::type surface_t::recalc_natural_slope( const koord k, sint8 &new_height ) const
{
	...
	// Per-vertex storage is canonical-by-construction, so the natural
	// slope is just what `get_height_slope_from_grid` reads back.  The
	// extra logic this function adds is rejecting the candidate when
	// it would collide with ground stacked above or below the tile.
	slope_t::type new_slope;
	sint8 candidate_height = 0;
	get_height_slope_from_grid(k, candidate_height, new_slope);
