...
public:
	enum class kind_t : uint8 {
		...
		slope,    ///< desc->get_slope_image_id(slope, snow)
	};

	way_image_slot_t() = default;

	static way_image_slot_t for_none()                                 { return way_image_slot_t(kind_t::none,  0,         false); }
	static way_image_slot_t for_flat(ribi_t::ribi r,    bool snow)     { return way_image_slot_t(kind_t::flat,  (sint16)r,  snow); }
	static way_image_slot_t for_slope(slope_t::type sl, bool snow)     { return way_image_slot_t(kind_t::slope, (sint16)sl, snow); }

	kind_t        kind()    const { return kind_; }
	ribi_t::ribi  get_ribi()  const { return (ribi_t::ribi)(uint8)key_; } ///< only meaningful when kind() == flat
	slope_t::type get_slope() const { return (slope_t::type)key_; }       ///< only meaningful when kind() == slope
	...
	bool operator==(const way_image_slot_t& o) const { return kind_ == o.kind_ && key_ == o.key_ && snow_ == o.snow_; }
	bool operator!=(const way_image_slot_t& o) const { return !(*this == o); }

private:
	way_image_slot_t(kind_t k, sint16 key, bool snow) : kind_(k), key_(key), snow_(snow) {}

	kind_t kind_ = kind_t::none;
	sint16 key_  = 0;       ///< ribi for flat, slope_t::type for slope; 0 for none
	bool   snow_ = false;
};
